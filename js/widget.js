...
wpWidgets = {
	...
	init : function() {
		...
		window.bpmh_already = new Array();
		
		$('.bpmh-widgets-sortables .widget').each( function() { 
			if( $(this).find('.widget-id').val().indexOf('bpmh_') != -1 )
				bpmh_already.push( $(this).find('.widget-id').val().replace(/\d+/g, '__i__') );
		});
		...
		$(document.body).bind('click.widgets-toggle', function(e) {
			...
			if ( target.parents('.widget-top').length && ! target.parents('#available-widgets').length && ! target.parents('#bp_my_home_widgets').length ) {
		...
		$('#bpmh-widgets').children('.widget').draggable({
			connectToSortable: 'div.bpmh-widgets-sortables',
			handle: '> .widget-top > .widget-title',
			distance: 2,
			helper: 'clone',
			zIndex: 100,
		...
		wpWidgets.bpmhsetdraggable();


		$('#bp_my_home_widgets').droppable({
			tolerance: 'pointer',
			accept: function(o){
				return $(o).parent().attr('id') != 'bpmh-widgets';
			},
			drop: function(e,ui) {
				ui.draggable.addClass('deleting');
				$('#removing-bpmhwidget').hide().children('span').html('');
			},
			over: function(e,ui) {
				ui.draggable.addClass('deleting');
				...
				if ( ui.draggable.hasClass('ui-sortable-helper') )
					$('#removing-bpmhwidget').show().children('span')
		...
		sidebars.sortable({
			...
			stop: function( event, ui ) {
				...
				/** BPMH customs 
				a bpmh widgets cannot be inactive!
				**/
				if( $widget.parent('div').prop( 'id' ).substr( 0, 5 ) != 'bpmh-' && 
					(  $widget.prop( 'id' ) &&
					   $widget.prop( 'id' ).indexOf( 'bpmh_' ) != -1  ) ) {
					$widget.remove();
					return;
				}
	...
	bpmhsetdraggable : function() {
		$('#bpmh-widgets .widget').each( function() {
			if( $.inArray( $(this).find('.widget-id').val(), bpmh_already ) != -1 )
	...
	save : function( widget, del, animate, order ) {
		...
		$.post( ajaxurl, data, function(r) {
			...
			if ( del ) {
				...
				/** BPMH customs 
				widget removed, so the #bp_my_home_widgets widget can now be draggable
				**/
				if( widget.attr('id').indexOf( 'bpmh_' ) != -1 ) {
					
					src = widget.find('.widget-id').val().replace(/\d+/g, '__i__')
					if( $.inArray( src, bpmh_already ) != -1 ) {
						bpmh_already.splice( $.inArray(src, bpmh_already),1 );
						wpWidgets.bpmhsetdraggable();
					}
				...
				/** BPMH customs 
				widget added, so the #bp_my_home_widgets widget is now not draggable
				**/
				if( widget.attr('id').indexOf( 'bpmh_' ) != -1 ) {
					
					src = widget.find('.widget-id').val().replace(/\d+/g, '__i__')
					if( $.inArray( src, bpmh_already ) == -1 ) {
						bpmh_already.push( src );
						wpWidgets.bpmhsetdraggable();
					}
